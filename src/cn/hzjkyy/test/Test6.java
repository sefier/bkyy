...
public class Test6 {
	public static void main(String[] args) throws InterruptedException {
		//组建基本环境
		Log applicationLog = Log.getLog("application" + "-" + "Test6");
		Explorer explorer = new Explorer("Test6");
		User user = new User("330902198209025837", "123456");
		...
		while(System.currentTimeMillis() < 1410225000000L){
			...
			//获取教练车信息
			applicationLog.record("系统开始获取教练车。");
			jlkGenerator = new JlcGenerator(user);
			jlkRequest = jlkGenerator.generate();
			jlcParser = new JlcParser();
			
			do {
				applicationLog.record("获取教练车...");
				response = mainTab.visit(jlkRequest);
				if(response.getStatusPanel().isSuccess()){
					jlcParser.parse(response.getResponseBody());
				}
			}while(!jlcParser.getStatusPanel().isSuccess());
			jlc = jlcParser.getJlcs()[0];
			kskm = jlcParser.getKskm();
			user.setKskm(kskm);
			applicationLog.record("获取教练车成功：" + jlc + ",考试科目：" + kskm);
		}
